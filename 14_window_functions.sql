...
-- 121 Add row numbers to all employees ordered by salary (highest to lowest).
SELECT E.FIRST_NAME, E.LAST_NAME, E.SALARY,
ROW_NUMBER() OVER(ORDER BY E.SALARY DESC)
FROM EMPLOYEES E
WHERE E.SALARY IS NOT NULL;

-- 122 Show each employee's salary along with the average salary of their department.
SELECT E.FIRST_NAME || ' ' || E.LAST_NAME, E.SALARY, 
AVG(E.SALARY) OVER(PARTITION BY E.DEPT)
FROM EMPLOYEES E
WHERE E.SALARY IS NOT NULL;
-- 123 Rank employees by salary within their department (highest salary = rank 1).
SELECT E.FIRST_NAME, E.SALARY,
RANK() OVER(PARTITION BY E.DEPT ORDER BY E.SALARY DESC)
FROM EMPLOYEES E
WHERE E.SALARY IS NOT NULL;
-- 124 Find the salary difference between each employee and the highest paid person in their department.
SELECT E.FIRST_NAME, ((MAX(SALARY) OVER(PARTITION BY DEPT)) - E.SALARY)
FROM EMPLOYEES E
WHERE E.SALARY IS NOT NULL; 
-- 125 Show each employee with the count of people in their department (including themselves).
...
-- 126 Display each employee's salary as a percentage of their department's total salary.
...
-- 127 Show the cumulative sum of salaries ordered by hire_date.
select SUM(E.SALARY) OVER(ORDER BY hire_date)
FROM EMPLOYEES E;
-- 128 For each employee, show their salary and the salary of the next person hired after them.
SELECT E.FIRST_NAME, E.SALARY, 
LEAD(E.SALARY) OVER(order by hire_date)
FROM EMPLOYEES E
WHERE E.SALARY IS NOT NULL;
-- 129 Show the first and last hire date for each department alongside each employee.
...
FROM EMPLOYEES E;
-- 130 Rank employees by hire_date (earliest = 1) and show only the first 2 hired in each department.
